...
//        }
    ...
    public Uri getItemContentUri(Context context) {
        ...
        LogUtils.d(TAG, "getItemContentUri, filePath = " + mAbsolutePath+
            ", projection = "+projection+
            ", where = "+where+
            ", baseUri = "+baseUri);
        Cursor c = context.getContentResolver().query(baseUri,
            projection,
            where,
            new String[]{mAbsolutePath},
            null);
        LogUtils.d(TAG, "getItemContentUri, cursor = " + c);
        if (c != null && c.moveToNext()) {
            int type = c.getInt(c.getColumnIndexOrThrow(MediaColumns._ID));
            if (type != 0) {
                long id = c.getLong(c.getColumnIndexOrThrow(MediaColumns._ID));
                LogUtils.d(TAG, "getItemContentUri, item id = " + id);
