...
public final class DrmManager {
     ...
     */
    public Bitmap overlayDrmIconSkew(Resources resources, String path, int iconId) {
        init(mContext);
        if (mDrmManagerClient != null && OptionsUtils.isMtkDrmApp()) {
            return OmaDrmUtils.overlapLockIcon(mDrmManagerClient, resources, iconId,  path);
        } else {
            return null;
        }
    }
    ...
    public boolean isRightsStatus(String path) {
        init(mContext);
        if (mDrmManagerClient != null && OptionsUtils.isMtkDrmApp()) {
            return mDrmManagerClient.checkRightsStatus(path, DrmStore.Action.TRANSFER)
    ...
    public void showProtectionInfoDialog(Activity activity, String path) {
        init(mContext);
        if (mDrmManagerClient != null && OptionsUtils.isMtkDrmApp()) {
            OmaDrmUtils.showProtectionInfoDialog(activity, mDrmManagerClient, path);
        }
    }
